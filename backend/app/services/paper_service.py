...
class PaperService:
    ...
    def search_papers(
        ...
        *,
        ...
        items = self._maybe_enrich_search_candidates(items, tag=tag, sort=sort)
        items = self._filter_papers(items, tag=tag)
        items = self._sort_papers(items, sort=sort)
        total = len(items)
        offset = (page - 1) * limit
        page_items = items[offset : offset + limit]
        saved_lookup = self.repository.get_saved_states([item.id for item in page_items if item.id is not None])
        return {
            "items": [self._serialize_paper(item, saved_states=saved_lookup.get(item.id or -1, set())) for item in page_items],
    ...
    def get_paper(self, paper_id: int) -> dict:
        ...
        saved_lookup = self.repository.get_saved_states([paper.id] if paper.id else [])
        return self._serialize_paper(
            paper,
            include_related=True,
            saved_states=saved_lookup.get(paper.id or -1, set()),
        )
    ...
    def summarize_paper(self, paper_id: int) -> dict:
        ...
        saved_lookup = self.repository.get_saved_states([paper_id])
        return self._serialize_paper(
            refreshed,
            include_related=True,
            saved_states=saved_lookup.get(paper_id, set()),
        )
    ...
    def list_saved_papers(self) -> dict:
        ...
        favorite_lookup = self.repository.get_saved_states([paper.id for paper in favorites if paper.id is not None])
        reading_lookup = self.repository.get_saved_states([paper.id for paper in reading if paper.id is not None])
        return {
            "favorite": [
                self._serialize_paper(paper, saved_states=favorite_lookup.get(paper.id or -1, set()))
                for paper in favorites
            ],
            "reading": [
                self._serialize_paper(paper, saved_states=reading_lookup.get(paper.id or -1, set()))
    ...
    def set_saved_state(self, paper_id: int, list_type: str, enabled: bool) -> dict:
        ...
        saved_lookup = self.repository.get_saved_states([paper_id])
        return self._serialize_paper(refreshed, saved_states=saved_lookup.get(paper_id, set()))
    ...
    def _serialize_paper(
        ...
        *,
        ...
        payload["saved"] = {
            "favorite": "favorite" in saved_states,
            "reading": "reading" in saved_states,
        }
        if include_related:
            payload["related_papers"] = self._build_related_papers(paper)
        return payload

    def _maybe_enrich_search_candidates(self, papers: list[Paper], *, tag: str, sort: str) -> list[Paper]:
        if not papers:
            return papers
        signal_tags = {"引用量高", "高被引", "新晋热门", "影响力强"}
        needs_signals = sort.startswith("citations") or tag in signal_tags
    ...
    def _filter_papers(self, papers: list[Paper], *, tag: str) -> list[Paper]:
        normalized = tag.strip()
        if not normalized:
            return papers
        return [paper for paper in papers if normalized in self.tag_service.build_tags(paper)]
